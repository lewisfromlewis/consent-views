...
Views <- readxl::read_xlsx("Views.xlsx", sheet=1) 

Viewslong <- Views %>% 
    select(-Type, -WillingYN, -Interviewer, -InPerson) %>% 
...
```{r}
Viewclusters <- Views %>% 
    select(-ID, -Type, -WillingYN, -Interviewer, -InPerson) %>% 
...
PCAplot <- fviz_cluster(kmViewPCA, data = ViewPCAredux)
print(PCAplot)
```